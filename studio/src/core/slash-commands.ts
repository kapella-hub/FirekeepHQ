...
export function createCommandRegistry(service: StudioService, integrations: CommandIntegrations = {}): CommandRegistry {
  ...
  commands.register({ name: "voice", summary: "Control Studio speech input and text-to-speech.", usages: ["/voice on|off|status"], execute: async (command) => { const action = command.args[0] ?? "status"; if (action === "on") await service.setVoice(true); else if (action === "off") await service.setVoice(false); else if (action !== "status") throw new Error(`unknown voice action: ${action}`); return notice("Voice", service.snapshot().voiceEnabled ? "On" : "Off", "neutral"); } });
