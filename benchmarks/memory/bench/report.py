...
def load_recall_counts(work_dir) -> dict:
    """Best-effort read of `<work>/recall_counts.json` — absent or malformed
    degrades to `{}` (the record then simply carries no counts, which
    `bench.dream_ab` reports as unverified rather than as evidence).

    Lives here, not in `bench.run`, because BOTH report-assembly paths must
    stamp these onto the published record: the one inside a full `bench.run`
    and the standalone `python -m bench.report --run-label X`. A record
    regenerated by the standalone path with no counts silently downgrades
    `bench.dream_ab` from a hard gate to a non-gating warning.
    """
    path = (work_dir or WORK_DIR) / "recall_counts.json"
    if not path.exists():
        return {}
    try:
...
def attach_recall_counts(scores: dict[str, dict], work_dir) -> dict[str, dict]:
    """Stamp each config's recall counts into its score dict, in place.

    This is the positive control `bench.dream_ab` reads: what the recall stage
    actually EXECUTED, carried inside the same dict the comparator compares, so
    a leg that recalled nothing cannot be reported as a flawless +0.0000.
    """
    counts = load_recall_counts(work_dir)
    for name, score in scores.items():
        entry = counts.get(name)
        if isinstance(entry, dict) and isinstance(score, dict):
            score["recall_counts"] = entry
    return scores


def results_path(run_label: str, results_dir: Path | None = None) -> Path:
    """Where a run-record for `run_label` is written.

    The label is sanitized HERE rather than validated at argument-parse time.
    Two reasons, both structural: the filename is built at two call sites
    (`bench.run._assemble_report` and `bench.report.main`), so a parse-time
    gate is a rule a third caller can forget while a sanitized construction
    site cannot fail; and validation would have to newly REFUSE labels that
    work perfectly today (`--run-label "full v1"` — a legal filename, already
    sanitized for the work dir), turning a fix into a usability regression.
    Sanitizing costs a digest suffix on the filename, which is not pure noise:
    it is what ties a published record to the `work/<label>/` directory that
    produced it, and what makes the filename injective in the raw label.
    """
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    return (results_dir or RESULTS_DIR) / f"{stamp}-{sanitize_label(run_label)}.json"


def _load_recall_per_question(config_name: str, work_dir=None) -> list[dict]:
    ...
    `work_dir` is the RUN-LABEL-scoped directory (`work/<label>/`); it defaults
    to the shared `WORK_DIR` only so the module stays usable standalone."""
    path = (work_dir or WORK_DIR) / f"recall_{config_name}.jsonl"
    if not path.exists():
        return []
    rows = []
...
def _load_qa_per_question(work_dir=None) -> list[dict]:
    ...
    path = (work_dir or WORK_DIR) / "qa_bench.jsonl"
    if not path.exists():
        return []
    rows = []
    for line in path.read_text(encoding="utf-8").splitlines():
...
def _load_per_question(config_names: list[str], work_dir=None) -> dict:
    per_question = {
        name: _load_recall_per_question(name, work_dir=work_dir)
        for name in config_names
    }
    per_question["qa"] = _load_qa_per_question(work_dir=work_dir)
    return per_question
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Recall/score/QA artefacts are per run label (see common.run_work_dir);
    # only the shared ingest ledger + ingest errors live at work/ root.
    ...
    # A record regenerated here must carry the same positive control a record
    # written by `bench.run` does — otherwise `bench.dream_ab` silently drops
    # from a hard gate to a warning for exactly the records an operator
    # regenerates by hand.
    attach_recall_counts(scores, work)
    ...
    per_question = _load_per_question(list(scores), work_dir=work)
    result = build_result(
        scores, qa, meta, ingest_errors=ingest_errors, per_question=per_question)

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    out_json = results_path(args.run_label, RESULTS_DIR)
