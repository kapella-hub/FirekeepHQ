...
### LLM endpoint selection (`cortex/app/llm.py`) — phase 1
...
**Converted in phase 1 (and deliberately not the rest):** `knowledge/classifier.py` and both `skills/synthesizer.py` chat calls — the docs→skills pipeline is the only path simultaneously on by default and currently failing, and the only one that stalls the solo worker for ~20 minutes. **Not converted:** `decision/synthesize.py` and `workers/sleep_cycle.py` (phase 2), `dreams/` (phase 3), and `engine/rag.py` synthesis/rerank plus `mcp_server.py` handoff (never this round — off by default, or the hot recall path). This split is intentional blast-radius management, not an oversight: **do not sweep the remaining seven sites in one unreviewed change**, since sleep_cycle's failure path writes the DLQ and rag.py is the hottest path in the system.
...
**Config:** `LLM_NATIVE_CHAT=auto|always|never` (default `auto`), `LLM_NATIVE_PROBE_TTL_SECONDS=600.0`, `LLM_NATIVE_BASE_URL=""` (only for an ollama-native backend whose root cannot be derived). Guards: `cortex/tests/test_llm.py` (54 tests) plus a `conftest.py` autouse reset of the module-global verdict cache — without it a verdict decided in one test leaks into later ones, order-dependently.
