...
*Usage-freshness follow-up (fixed 2026-08-02):* MCP `skill_recall` sends `record_recall=true`; the skills route records only the final post-filter result IDs, best-effort, through the app's existing Redis client. No-flag `GET /skills` remains a read-only listing path, so briefing/list/dashboard impressions cannot manufacture freshness. Semantic PATCHes to `content`, `trigger`, or `symptoms` now embed the merged semantic text before a single full-point Qdrant upsert, preserving unknown lifecycle/provenance payload fields; an embedding failure performs no write. Status/staleness-only PATCHes remain payload-only.
