...
### LLM endpoint selection (`cortex/app/llm.py`) — phases 1–2
...
**Selection is derive-then-probe with an explicit override.** Strip a *required* `/v1` suffix off `LLM_BASE_URL` to get a native root (no suffix → never native), confirm it with one `GET {root}/api/version` accepted only on 2xx **and** a JSON object carrying a `version` key (a catch-all proxy answering 200 must not be mistaken for ollama), and **fail toward `/v1` on anything unconfirmed** — `/v1` is the contract, native is the optimisation. A vLLM/LiteLLM/OpenAI backend simply never confirms and is unaffected; its body also stops carrying ollama-only vendor flags, which real OpenAI 400s on (`dreams/synthesize.py` sends them unconditionally today and gets away with it only because `DREAM_ENABLED=false`). The verdict is cached per root, positive for `LLM_NATIVE_PROBE_TTL_SECONDS` (600s) and negative for `min(60, ttl)` — the asymmetry exists so a probe that fails transiently while ollama restarts cannot pin every call onto the slow path for ten minutes. A pre-generation **4xx (400/404/405/501)** demotes the cached verdict and retries once on `/v1`, so an older ollama that rejects `think` degrades instead of failing hard; **5xx and timeouts propagate unretried** because they may arrive mid-generation. `llm.chat` RAISES on transport/HTTP failure and never swallows — `classifier.py` inspects the caught exception to choose between `corpus_only` and `failed`, and a helper that returned an empty result would collapse that distinction.
...
**Repointing `LLM_BASE_URL` at `http://ollama:11434` is NOT a valid workaround.** Eleven chat sites concatenate `/chat/completions` onto that variable and **three embedding sites concatenate `/embeddings`** (`db/vector.py`, `workers/reembed.py`), so repointing breaks every memory write. `config.py:205-216`'s dreams caveat still recommends it; that advice is safe only for a deployment where nothing else uses the variable, i.e. no deployment.

**Converted so far (and deliberately not the rest):** phase 1 — `knowledge/classifier.py` and both `skills/synthesizer.py` chat calls (the docs→skills pipeline is the only path simultaneously on by default and currently failing, and the only one that stalls the solo worker for ~20 minutes); phase 2 — `decision/synthesize.py` (below). **Not converted:** `workers/sleep_cycle.py` (deferred out of phase 2 — see below), `dreams/` (phase 3), and `engine/rag.py` synthesis/rerank plus `mcp_server.py` handoff (never this round — off by default, or the hot recall path). This split is intentional blast-radius management, not an oversight: **do not sweep the remaining six sites in one unreviewed change**, since sleep_cycle's failure path writes the DLQ and rag.py is the hottest path in the system.
...
**Config:** `LLM_NATIVE_CHAT=auto|always|never` (default `auto`), `LLM_NATIVE_PROBE_TTL_SECONDS=600.0`, `LLM_NATIVE_BASE_URL=""` (only for an ollama-native backend whose root cannot be derived). Guards: `cortex/tests/test_llm.py` (54 tests) plus a `conftest.py` autouse reset of the module-global verdict cache — without it a verdict decided in one test leaks into later ones, order-dependently. Phase 2 adds 9 cases to `cortex/tests/test_decision_synthesize.py` (the first to exercise `_llm_suggest` at all — every pre-existing case patched it out, which is how the silent-success bug survived review) and two compose/`.env.example` default-drift guards in `test_decision_config.py`.
...
**`think:false` is necessary and NOT sufficient — the completion budget is the other half.** Ollama honours `think` / `chat_template_kwargs.enable_thinking` on its **native `/api/chat`** and silently **ignores both on `/v1/chat/completions`**, which is where `synthesize()` posts because `LLM_BASE_URL` ends in `/v1`. Measured live against ollama 0.17.5 with the production request body, 3 probes of 3: HTTP 200, `finish_reason='length'`, `completion_tokens=700` (exactly the old hardcoded cap), **content length zero**, reasoning length ~3200 chars; the identical call at `max_tokens=4000` returned correct JSON. Field impact before the fix: 2 of 3 clusters produced **zero** insights, reproducibly, across two full runs — and since a zero-insight cluster is never added to `dreams:consolidated`, those clusters were re-selected and re-attempted on every subsequent run, a treadmill that could never succeed. `synthesize._MAX_COMPLETION_TOKENS` is therefore **4000**, deliberately a named module constant rather than a config field or a function of `DREAM_MAX_INSIGHT_CHARS`: that setting caps insight *content* (800 chars, enforced by `parse_insights`) while the tokens this budget must cover are overwhelmingly reasoning the content cap knows nothing about, and `build_request_body` is a pure function shared with `profile.py` with no `Settings` in scope. The flags stay — they are correct and do work against `/api`. **Known interaction:** on `/v1` the reasoning now actually gets generated instead of being truncated, so worst-case wall time rises against `DREAM_SYNTH_TIMEOUT_SECONDS=45.0` (the only control that binds under `--pool=solo`); a slow CPU backend can now time out where it previously returned fast-and-empty. Both yield zero insights, but the timeout is loud and `GET /dreams` now reports `degraded` rather than `ok`. The real remedy for such a deployment is pointing `LLM_BASE_URL` at `/api`, where `think:false` works and 22.5s is the measured latency — not raising the timeout without a measurement behind it.
