"""Stop core — presence deregister (race-guarded) + final git snapshot.

Ports scripts/debrief.sh: deregister presence UNLESS a newer session registered
within the last 5 s (else a just-started session loses its presence), push a
final workspace snapshot to Bridge scratch, and return the completion reminder.
"""
...
def run(payload: dict) -> dict:
    ...
    # 1. Deregister — unless a newer session registered <5s ago. Shared keying
    #    authority: firekeep_client.state.should_deregister/clear_registered
    #    (SP1b Task 19 seam reconciliation) -- the SAME scratch key
    #    session_start.py's mark_registered writes and the sidecar's
    #    independent registration guard reads, so a mixed hooks+sidecar
    #    composition for one agent_id can't clobber each other.
    do_dereg = state.should_deregister(agent, profile=profile)
    state.clear_registered(agent, profile=profile)  # consume the mark either way (bash parity)
    ...
    # 2. Final workspace snapshot -> Bridge scratch (best-effort).
    try:
        _mcp.call_tool(
            "bridge", "ctx_update",
    ...
    # 3. Structural capture: enqueue a distill job so the session's durable facts
