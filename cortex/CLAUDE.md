...
- **API key authentication**: shared per-key ASGI validator (auth/asgi.py FirekeepKeyAuthMiddleware) over the whole app when AUTH_ENABLED=true; skips /health, /version, /docs, /redoc, /openapi.json, /dashboard; fail-closed 503 when Redis DB 7 is unreachable while enabled
