...
│   ├── observe.py       # ProcedureObserver — the recognise/observe/warn stage called from AgentGatewayService.decide(). Holds NO vector client (I5)
...
- `GET /procedures` — Living Procedures per-skill rollup (see the root `CLAUDE.md`'s Living Procedures subsection): `{procedures: [{skill_id, trigger, observable_steps, executions, steps, proposals}], count, specs_total}`, sorted by `(-executions, skill_id)`. **`require_scope("memory:read")`.** `steps` is the per-step stats dict the nightly pass writes (seven int keys: `observed`/`skipped`/`executions` for Tier A, `observed_scored`/`observed_success`/`skipped_scored`/`skipped_success` for Tier B); `proposals` are advisory findings a human accepts by PATCHing the skill. A skill appears only if it has ≥1 `file_glob` spec in the matcher index — the rollup groups the index rather than scanning skills, so an all-`unobservable` skill is absent rather than present with `observable_steps: 0`. Router mounted only when `PROCEDURE_ENABLED=true` (the `/dreams` + `/collectors` precedent — a disabled deploy 404s rather than returning a disabled-shape body); route paths are unprefixed.
- `GET /procedures/{skill_id}/executions?limit=50` — The receipts behind those numbers: `{executions, count}` where `count` is the pre-limit total and each row is a live execution record (`exec_id`, `skill_id`, `session_id`, `agent_id`, `adapter`, `opened_at`, `last_seen_at`, plus a parsed `observed` map). `adapter` is a TRANSPORT class (`shell-hook`|`mcp`|`rest`), not a runtime, and must never be used to infer observability. **`require_scope("memory:read")`.**
