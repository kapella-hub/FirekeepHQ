...
├── llm.py             # Chat endpoint selection (ollama native /api/chat vs /v1), body build + response normalise
...
└── dreams/            # Automated memory consolidation + person profiles (round 1 — additive only)
    ...
    ├── synthesize.py     # The one LLM call for cluster insights (think:false, strict JSON)
...
- `LLM_NATIVE_CHAT` (default `auto`), `LLM_NATIVE_PROBE_TTL_SECONDS` (default `600.0`), `LLM_NATIVE_BASE_URL` (default `""`) — chat-endpoint selection (`app/llm.py`). Ollama honours `think:false` on its native `/api/chat` and IGNORES it on `/v1/chat/completions`; measured 2026-08-04 on the VPS (qwen3:4b, 4 vCPU), same document: **83.19s on `/v1` vs 4.00s native**. `auto` derives a root by stripping a required `/v1` suffix and confirms it with one cached `GET {root}/api/version` (2xx **and** a JSON object with a `version` key), failing toward `/v1` on anything unconfirmed. Do NOT "fix" this by repointing `LLM_BASE_URL` at the native root: three embedding call sites concatenate `/embeddings` onto the same variable. See the root `CLAUDE.md`'s "LLM endpoint selection" subsection for the full write-up, the phase plan (which call sites are converted and which are deliberately not), and the timeout reasoning.
