...
- **Skill staleness**: MCP `skill_recall` opts into usage recording for the final returned skill IDs, incrementing `memory:access_counts` and HSETing `memory:last_recalled`; ordinary `skill_list`, dashboard browsing, and automatic briefing impressions do not count. `flush_last_recalled` drains timestamps to Qdrant before `skill_staleness_pass` flags active skills unrecalled past `SKILL_STALE_AFTER_DAYS` (default 90) as `stale=True` and un-flags re-recalled ones. It never changes `skill_status` or deletes. Semantic PATCH fields (`content`, `trigger`, `symptoms`) are re-embedded before a full-point upsert; lifecycle-only edits stay payload-only.
...
- **Memory Agent**: Autonomous knowledge custodian — Celery periodic task (default 6h) runs duplicate detection/merge, deep contradiction scan, cluster coherence/domain reclassification, access-count flush, explicit-recall timestamp flush, and skill-staleness review, followed by the separately gated skill-synthesis catch-all. Its graph-orphan delete pass runs only with `GC_PURGE_ENABLED=true`, so the default maintenance paths perform no automatic hard deletion. There is no confidence-decay pass. Redis SETNX prevents overlapping runs; per-pass error isolation keeps one failure from stopping the rest.
