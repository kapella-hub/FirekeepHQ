...
- The dex registry: dexes are the domain indexes the Keep understands, listed and switched with `firekeep dex list|add|remove` against `~/.firekeep/dexes.json`. Both wheels ship bundled and checksum-verified with every release — **registration gates activity, not installation**. Existing installs keep symdex across an update; fresh installs opt in — see [docs/guides/dexes.md](docs/guides/dexes.md)
