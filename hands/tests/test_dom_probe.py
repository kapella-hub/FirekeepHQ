...
# `__hands` is a `const` Browser.py prepends; here it is a global for the
# same reason `document` is: nothing else defines it in this scope).
_RUNNER_PREAMBLE = r"""
...
global.__hands = %(hands)s;
"""


def _run_probe(specs: list[dict], hands: dict) -> dict:
    probe_source = _PROBE_PATH.read_text(encoding="utf-8")
    script = _RUNNER_PREAMBLE % {"specs": json.dumps(specs), "hands": json.dumps(hands)}
    # The probe file is itself a single `(function () { ... })()` expression,
    # so its value can be assigned directly rather than re-wrapped.
    script += f"\nvar __result = {probe_source};\nprocess.stdout.write(JSON.stringify(__result));\n"
...
def test_focus_returns_ok_and_rect_for_a_known_ref() -> None:
    specs = [
        {"tag": "INPUT", "text": "", "attrs": {"data-hands-ref": "d1"},
         "rect": {"left": 5, "top": 6, "width": 100, "height": 20}},
    ]
    result = _run_probe(specs, {"op": "focus", "ref": "d1", "max_nodes": 200})

    assert result["ok"] is True
    assert result["rect"] == [5, 6, 100, 20]


def test_focus_returns_not_ok_for_an_unknown_ref() -> None:
    result = _run_probe([], {"op": "focus", "ref": "d999", "max_nodes": 200})
