...
**Amendments after implementation (2026-09-05):** (a) `hands_browser click`/`fill` are classified through `policy.classify` with a synthetic `Control` built from the DOM probe result (role/name/value/href/rect) and the current URL — a "Send" or "Place order" button in a web page needs the same permit as a native one; the probe reports `input[type=password]` as role `password`, which the session maps to `PasswordBox` (credential). (b) `type` actions are classified against a focus hint — the control last targeted by `click`/`invoke`/`set_value` — because routing has no control for a bare `type`; best-effort, documented. (c) The ledger copy of a `type`/`set_value` action is redacted to `<redacted:credential>` when the step is classified credential or the target is a `PasswordBox`; the challenge hash still covers the real action. (d) A relay lease held by another session makes `hands_task_start` refuse with the holder and expiry in the error text; no override in PR1 (the guide discloses the 30-minute lockout after a crash).
...
**Doctor remedy (added 2026-09-05 after the Task 6 review):** with phone approvals off by default, a machine whose chord listener cannot install has NO approval path. `_check_hands()` in `client/firekeep_client/cli.py` must return `("hands", "warn", …)` when `/health` reports `listeners.chord == "unavailable"` and `listeners.phone != "active"`, and the text must name the remedy: on macOS grant Input Monitoring to the broker (System Settings → Privacy & Security → Input Monitoring → the kit's python) and restart it; on Windows re-run `firekeep hands enable` (recreates the logon task); or opt in to phone approvals with `firekeep hands config set phone_approvals true` after reading the guide's trust-boundary note. Add the test to `client/tests/test_cli_hands.py` (health dict with `chord: "unavailable"`, `phone: "off"` → `warn` and "Input Monitoring" in the text).

- [ ] **Step 1: Write the docs and the doctor remedy.** **Step 2:** `python -m pytest tests -q -k "docs or guide or claude_md"` (whatever doc-consistency tests exist) and `cd client && python -m pytest tests/test_cli_hands.py -q` → PASS. **Step 3: Commit** — `git commit -m "docs(hands): guide, registry role, kit CLI and doctor rows, threat model, CLAUDE.md"`.
