...
| **Local dev story** | One command (`curl -fsSL https://firekeep.ai/latest/install.sh \| sh`, or `./install` from a checkout) installs the portable `firekeep-client` kit (one `[identity]` + `[server]` connection in `~/.firekeep/config`, `firekeep-shim` transport, hook cores, sidecar); agents connect to the Firekeep server over HTTP(S) with keyed, attributed identity |
