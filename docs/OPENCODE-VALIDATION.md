...
| 7 | `session.deleted` → stop core | **CONFIRMED (mechanism since changed — see below)** | Bus log: `type=session.deleted unsubscribing` at run end; Relay `/presence` showed the agent deregistered after the run (the stop core's race-guarded deregister). |
| 7a | `session.deleted` → stop **and** session_end | **WIRED + unit-verified; live re-validation OUTSTANDING** | 2026-07-28 the deregister moved out of the `stop` core into `session_end`, because on claude `Stop` fires per turn and deregistering there deleted presence after turn 1. opencode is unaffected in behaviour — its `session.deleted` is real session end — but the plugin now has to drive BOTH cores to keep row 7's outcome. Verified: the rendered plugin passes `node --check` and dispatches all six cores; two regression tests pin the `session.deleted` branch and fail if the `session_end` dispatch is dropped (mutation-tested). **NOT verified: a live `opencode run` confirming Relay `/presence` is still empty afterwards** — that needs a machine with both opencode and a rendered Firekeep client, and row 7's original evidence no longer describes the shipped code path. |
