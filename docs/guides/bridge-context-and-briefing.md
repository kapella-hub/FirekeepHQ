...
**Bridge:** `ctx_start_session` (MCP tool) and `SessionManager.start_session()` gained an optional `briefing_id`, threaded into the session hash to link a briefing to the session it originated — closing the strategy-tip A/B feedback loop (a carryover from the 2026-07-08 team-activity-hub master design). `GET /briefing`'s `instructions` field renders the server-minted `briefing_id` into every branch's suggested `ctx_start_session(goal=..., briefing_id='<id>')` call, so an agent that follows the printed instruction supplies it; the join in `GET /patterns/effectiveness` (see Feedback Loop below) only closes for sessions whose agent actually passed it along — Bridge has no way to force that, it's a documented instruction, not an enforced contract.
...
**Outcome truth PR5 (2026-08-26) — a `grading_nudge` section, dark until T0, and a `member_token` beside `experiment_group`.** Pre-registered in [`docs/superpowers/specs/2026-08-26-outcome-truth-pr5-controlled-nudge-design.md`](../superpowers/specs/2026-08-26-outcome-truth-pr5-controlled-nudge-design.md). The `GET /briefing` envelope gains a top-level `experiment_group` field beside `briefing_id` (the same arm `auth/experiment.py::experiment_group` computes from the verified `member_id`, shared by construction with Bridge's stamp below — delivered arm and recorded arm cannot diverge). A new `grading_nudge` section (`cortex/app/briefing/sections.py::grading_nudge_section`) is registered last among the section builders and is the last thing `render.py` emits before `=== END BRIEFING ===`; it is dark by default (`GRADING_NUDGE_ENABLED=False`) and, even once flipped, composes the pre-registered treatment text for the treatment arm only — control and None-arm get no section at all. It is withhold-on-record-failure: if the server-side `rp:nudge_shown:{briefing_id}` receipt cannot be written, the section is not shown, the same precedent the strategy-tips A/B already set for an unrecorded exposure. Separately, `bridge/app/session.py::start_session` now stamps `member_token` (`auth/experiment.py::member_token` — a one-way `sha256(owner_member)` prefix, 12 hex chars) beside `experiment_group` at the same resolution point, so the two can never disagree about which member a session belongs to; it rides the `session_start` replay payload the same way `experiment_group` already does (see the `EvalResult.experiment_group` entry in `replay-evals-patterns.md`) and lands on the parsed `EvalResult` alongside `briefing_id`, giving the PR5 readout (`arm_comparison`, `knowledge-autopilot.md` §6) a member key it did not have before — a record with no token classifies `unknown` for member-level analysis.
