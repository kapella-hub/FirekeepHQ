...
- `stop` (Stop) — guided completion: final workspace snapshot, distill/tasks/lease reminders, the skill-ladder feedback reminder (call `memory_feedback` on a recalled skill's id — that is the evidence that promotes or demotes it on the nightly ladder pass, [`knowledge-autopilot.md`](knowledge-autopilot.md) §9), and presence deregistration (race-guarded against a newer session's registration).
