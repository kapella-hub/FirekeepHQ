...
exists it fire-and-forgets a background `firekeep update` (`autoupdate.maybe_spawn` →
`subprocess.Popen([venv/firekeep, "update"], **background.popen_kwargs())`: POSIX
`start_new_session=True`; Windows `CREATE_NO_WINDOW | CREATE_NEW_PROCESS_GROUP` — a **hidden**
console, deliberately not `DETACHED_PROCESS`, see "Background workers on Windows" under the
symdex auto-index section). This automates the SAME operation a user runs by hand mid-session, and
...
The five bash hooks are retired; the adapter wires stdlib Python hook cores at install (Claude `settings.json`, kiro inline hooks, OpenCode via a rendered JS plugin bridge, Pi via the `firekeep-pi` npm extension; Codex and the generic runtime have no hook surface):
...
- `stop` (Stop) — guided completion: final workspace snapshot, distill/tasks/lease reminders, the skill-ladder feedback reminder — verbatim, "If a recalled skill guided this work, call memory_feedback with its id (useful=true/false) — that is the evidence that promotes or demotes it." ([`knowledge-autopilot.md`](knowledge-autopilot.md) §9) — and presence deregistration (race-guarded against a newer session's registration).
...
Presence registration/heartbeat/periodic snapshots/deregistration for Claude Code — and for kiro, OpenCode and Pi, which wire the same cross-runtime hook cores to their own lifecycle events (kiro and OpenCode expose no compaction event to wire; Pi exposes `session_before_compact`, which the bridge wires but which is not yet validated, so only Claude Code's `precompact` is a proven path) — are owned directly by the hook cores above: `session_start` registers, `prompt` heartbeats, `stop` deregisters. The **sidecar** (`firekeep-sidecar`, one daemon per agent identity) is the *intended* presence owner for MCP-only runtimes with no hook lifecycle at all (Codex today), but nothing currently spawns it automatically — a Codex user has no presence path unless they run `firekeep-sidecar` by hand. The retired launcher is replaced by the `FIREKEEP_AGENT_ID` env override: set it in the process environment to run differently-identified agents from one machine (it overrides `[identity] agent_id`).
...
**Pi adapter (`client/firekeep_client/adapters/pi.py`):** renders three surfaces — the `firekeep-pi` npm package added to the `packages` array of `~/.pi/agent/settings.json`, a `~/.firekeep/pi-extension.json` sidecar naming the kit's venv interpreter, and the firekeep instruction block upserted into the user's global `~/.pi/agent/AGENTS.md` (marker-delimited, claude-CLAUDE.md precedent). Unlike every other adapter here there is **no `mcp` key to write**: Pi ships no MCP client (its docs list MCP with sub-agents and permission popups as "extension or package; not built-in"), so this adapter delivers the HOOK surface only — an agent on Pi cannot call `memory_recall` or `ctx_update` unless the user separately installs a community MCP extension, which Firekeep neither ships nor controls. `tests/adapters/test_pi.py::test_renders_no_mcp_key` pins that boundary so it fails loudly if Pi ever gains built-in MCP.

The bridge is TypeScript published to npm (`firekeep-pi`, sources at `client/pi/`), loaded by Pi through jiti with no build step, and it maps SEVEN cores: `session_start`→`session_start` (skipping `reason === "reload"`, which is an extension reload and would double-register presence), `before_agent_start`→`prompt`, `tool_call`→`pre_tool`, `tool_result`→`post_tool`, `agent_settled`→`stop` (Claude's per-turn Stop equivalent), `session_before_compact`→`precompact`, `session_shutdown`→`session_end`. Tool names are translated to the Claude shapes the cores expect (`edit`/`write`→`Edit`/`Write`, `bash`/`powershell`→`Bash`) and Pi's `path` argument becomes `tool_input.file_path`; ungated tools are never forwarded, so they cost no subprocess.

Two capabilities put Pi above every other non-Claude runtime, both **VALIDATED live on Pi 0.84.4** (`docs/PI-VALIDATION.md`, 16/16 assertions):
- **The briefing and per-prompt recall reach MODEL CONTEXT, not the console.** Pi's `before_agent_start` may return a replacement `systemPrompt`; the harness captures `context.systemPrompt` inside the provider and finds the injected text there. This is the exact thing the OpenCode bridge above cannot do. Deliberately NOT using the dispatcher's `hookSpecificOutput.additionalContext` decoration — that shape is Claude Code's and `_MODEL_CONTEXT_RUNTIMES` stays `{"claude"}` on purpose; Pi's channel is its own.
- **The pre-edit gate is a HARD gate.** Pi's `tool_call` takes `{block: true, reason, terminate}` as a first-class return, so the bridge blocks on the dispatcher's `--block-exit 2` without throwing. Proven as a matched pair: the target file is absent at exit 2 and present at exit 0.

Caveats: `precompact` is wired but unexercised, so the matrix cell reads "wired, unverified" rather than borrowing Claude's value; every hook degrades to a no-op when no interpreter can import `firekeep_client` (availability over enforcement, same contract as the other bridges); and the auto-render gate is `~/.pi/agent` existing (`app_present()`, the claude-desktop precedent), so `firekeep install` with no `--runtime` writes nothing on a machine that has never run Pi.
...
**Background workers on Windows: a hidden console, never no console (`client/firekeep_client/background.py`, 2026-08-25).** All four fire-and-forget workers the `session_start` core launches — this auto-index, the docdex and maildex syncs, and the auto-update — take their `Popen` kwargs from one helper, `background.popen_kwargs()`, and the Windows half of it is `CREATE_NO_WINDOW | CREATE_NEW_PROCESS_GROUP`, **not** `DETACHED_PROCESS`. They used `DETACHED_PROCESS` ("no console") until a Windows Terminal window started opening on every session start for as long as the index or sync ran. The mechanism: `sys.executable` inside a venv (and the `firekeep.exe` console script) is a *launcher* that re-spawns the real interpreter as a child, and process-creation flags are not inherited — the launcher ran console-less, so its child, a console application whose parent has no console, was handed a brand-new console by Win32, which Windows 11 delegates to Windows Terminal. It was reproduced in isolation with `python -c "time.sleep(4)"` under each flag set (DETACHED pops the window; CREATE_NO_WINDOW does not) — `git`, the first suspect, was innocent: it inherited the console the interpreter had already been given. A hidden console is inherited by every descendant (base python, git, the updater's `powershell`), and it is the worker's own, so the hook exiting still closes nothing it holds. Guards: `client/tests/test_background.py` asserts the contract for BOTH platforms on every CI runner (the flags are literals, so Linux can check the Windows half) and that each of the four `maybe_spawn`s routes through the helper rather than re-spelling the flag; the four modules' own spawn tests assert `CREATE_NO_WINDOW` present and `DETACHED_PROCESS` absent.
...
## Hands (client kit — `firekeep hands`, `firekeep_client.cli.cmd_hands`)

The desktop operator, and the registry's first `role: "capability"` entry. Everything about what it does to the machine, the six protected classes, the approval broker and the honest limits is [`hands.md`](hands.md); what belongs here is the kit's own half.

**`enable`/`disable` are the kit's; everything else is a translator.** `cmd_hands` handles the two actions that touch the venv and the registry, and passes `status|allow|chord|config|evidence` straight through to `firekeep_hands.cli.main` with the words that followed them — imported lazily, so a kit without the wheel keeps every other command working and says `run \`firekeep hands enable\`` instead of raising. Deliberately absent from every one of those actions: any way to approve or deny a pending permit. A CLI a script could drive is exactly the hole the broker's input listener exists to close.

| Command | What it does |
|---|---|
| `firekeep hands enable --from <checkout>/hands` | pip-install the wheel into the kit venv, prove `firekeep_hands` imports, register `hands`, install the broker autostart (`--no-autostart` skips the last) |
| `firekeep hands disable [--purge]` | unregister, remove the autostart; `--purge` also deletes `~/.firekeep/hands` (config, allowlist, Chrome profile, every evidence ledger) |
| `firekeep hands status` | platform, backend + OS permissions, broker/chord/phone, policy counts, last task |
...
**Not bundled, and not PyPI-installable yet.** Unlike the three dex wheels, `firekeep-hands` does not arrive with a release — the pyobjc set alone is tens of megabytes and a capability with real OS permissions is opt-in on principle. `HANDS_PYPI_PUBLISHED` is still `False` (the squat guard, 2026-09-05): until the name is published through the `pypi-hands` trusted publisher, a bare `enable` and `--pypi` both refuse rather than `pip install`-ing a name a third party could claim, and `--from <checkout>/hands` is the only working path. `HANDS_WHEEL_SPEC` (`firekeep-hands>=0.1,<0.2`) is the one place in the kit that names the source.

**Doctor gains one `hands` row**, appended by `_check_dexes` only when `hands` is in the registry, alongside the existing per-dex rows: `fail` when the wheel is missing, `warn` when the broker is not answering `/health`, `warn` when the broker is up but nothing can approve (chord listener `unavailable` and phone not `active` — a machine in that state refuses every protected step, correctly and silently), `ok` otherwise, showing the chord and the phone state. `read_broker_health()` is stdlib-only and reads `~/.firekeep/hands/broker.json` for the loopback port and bearer token the running broker wrote; no file or a refused connection both mean "not running", so doctor never hangs on it.

**Import boundary:** the kit spine never imports `firekeep_hands` at module level. `cmd_hands`, `_run_hands_broker` and the wheel delegation all import inside the function, and the doctor row reaches the broker over loopback HTTP rather than through the wheel — the same rule that keeps tree-sitter out of every PreToolUse gate (`client/tests/test_import_boundary.py`).
