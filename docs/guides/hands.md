...
| The Keep round trip: `action_before` against the real Keep returned an action id with decision `allow`, and `action_after` came back `recorded: true` — after the fix that replaced the rejected `action_type` value (the first attempt proved the review's finding: an error string, no id) | **Verified** | 2026-09-05, Windows 11, real Keep over the tailnet |
